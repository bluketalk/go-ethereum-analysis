	...
	for {
		...
		case c := <-srv.addpeer:
			...
			if err == nil {
				...
				log.Info("peer name = ", "name",c.name)
				log.Info("Adding p2p peer", "name", c.name, "addr", c.fd.RemoteAddr(), "peers", len(peers)+1)
...
func (srv *Server) SetupConn(fd net.Conn, flags connFlag, dialDest *discover.Node) error {
	...
	if err != nil {
		c.close(err)
		log.Info("Setting up connection failed", "id", c.id, "err", err)
		srv.log.Trace("Setting up connection failed", "id", c.id, "err", err)
	}
	log.Info("Setting up connection success", "id", c.id)
	return err
}
